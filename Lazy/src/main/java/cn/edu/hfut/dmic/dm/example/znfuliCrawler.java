 ...
 *
 */
public class znfuliCrawler extends BreadthCrawler {
	...
	static {		/*
		 * 1 1 jishuzhai 技术宅 0 90 2 2 znpd 宅男 0 0 3 3 nvshen 女神 0 90 4 4 film 电影
		 * 0 1 5 5 zonghe 资讯 0 0 6 6 acgzh ACG综合区 0 0 7 7 huodong 福利活动 0 0 8 8
		 * baike 宅男百科 0 0
		 */
	 ...
	 */
	public znfuliCrawler(String crawlPath, boolean autoParse, int id, Map.Entry<String, String> e) {
		super(crawlPath, autoParse);
		String url = "https://www.zhainanfulishe.net/"+e.getKey()+"/page/" + id;
		this.addSeed(url);//
	...
	public void visit(Page page, CrawlDatums next) {
		if (page.matchUrl("https://www.zhainanfulishe.net/.*/page/.*")) {
			System.out.println(page.getHtml());
			Elements as = page.select(".home-blog-entry-text.clr>h3>a");
			...
			if (page.matchUrl("https://www.zhainanfulishe.net/.*html")) {
				String title = page.select("#breadcrumbs>h1").text();
				String content = page.select(".single-text").html();
				String tag = page.select(".sg-tag>a").html();
	...
	public static void execute(int pagesize) throws Exception {
		for (Map.Entry<String, String> e : t.entrySet()) {
			int i = 1;
			while (i > 1) {
				znfuliCrawler crawler = new znfuliCrawler("crawl", true, i,e);
